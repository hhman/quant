...
class CSVDataLoader:
    ...
    def __init__(
        ...
        self.expected_category_files = [
            f"{self.membership_dir}/{filename}.txt"
            for filename in self.MEMBERSHIP_FILE_MAP.values()
        ]
    ...
    def _process_index_file(self, csv_path: Path) -> tuple[str, pd.DataFrame] | None:
    ...
    def _process_directory(
        self,
        directory: Path,
        handler: Callable[[Path], tuple[str, pd.DataFrame] | None],
    ...
    def _process_stock_file(self, csv_path: Path) -> tuple[str, pd.DataFrame] | None:
        ...
        mask = (raw_df["交易日期"] >= self.start_date) & (
            raw_df["交易日期"] <= self.end_date
        )
        ...
        industry_series = (
            raw_df.set_index("交易日期")[self.INDUSTRY_COLUMN]
            .sort_index()
            .ffill()
            .bfill()
        )
    ...
    def _collect_membership_categories(self, code: str, df: pd.DataFrame) -> None:
        ...
        for column, filename in self.MEMBERSHIP_FILE_MAP.items():
            ...
            self._collect_category(
                code,
                series,
                lambda value, fname=filename: f"{self.membership_dir}/{fname}.txt"
                if value == 1
                else None,
            )
    ...
    def _write_category_files(self) -> None:
        ...
        for relative_path, records in self.category_records.items():
            ...
            delimiter = "\t" if relative_path.startswith(self.membership_dir) else ","
    ...
    def _save_industry_mapping(self) -> None:
        ...
        self.industry_mapping_file.write_text(
...
def parse_args() -> argparse.Namespace:
    """解析 CLI 参数"""
    parser = argparse.ArgumentParser(description="Step 0.1: 日线数据清洗")
    parser.add_argument(
    ...
    parser.add_argument("--stock-dir", type=str, required=True, help="股票CSV数据目录")
    parser.add_argument("--index-dir", type=str, required=True, help="指数CSV数据目录")
