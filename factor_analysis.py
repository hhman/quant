...
def neutralize_industry_marketcap(
    ...
    # 行业哑变量
    X_dummies = pd.get_dummies(group[industry_col], drop_first=True, prefix=industry_col, dtype=float)
    if X_dummies.empty:
        print(f"[neutralize_industry_marketcap] {dt} 行业只有一个类别，回归仅使用对数市值")
    ...
    for factor_name in factor_list:
        ...
        if before_drop > 0:
            ...
            if drop_ratio > 0.1:
                print(f"[neutralize_industry_marketcap] {dt} {factor_name} dropna 丢弃 {before_drop - len(reg_data)}/{before_drop} ({drop_ratio:.1%})")

        n_params = len(X_styles.columns) + 2  # 截距 + 因子 + 风格
        if len(reg_data) < n_params:
            print(f"[neutralize_industry_marketcap] {dt} {factor_name} 样本 {len(reg_data)} 少于所需 {n_params}，置 NaN")
        ...
        try:
            ...
            if model.rsquared < 0.1:
                print(f"[neutralize] {dt} {factor_name} R²={model.rsquared:.3f}，拟合质量较低")
            elif model.rsquared < 0.05:
                print(f"[neutralize] {dt} {factor_name} R²={model.rsquared:.3f}，拟合质量很差")
        except Exception as e:
            print(f"[neutralize_industry_marketcap] {dt} {factor_name} 回归失败：{e}，置 NaN")
            # result[factor_name] 已经是NaN，无需操作
    ...
    for factor_name in factor_list:
        ...
        for ret_col in ret_list:
            ...
            if before_drop > 0:
                drop_ratio = (before_drop - len(reg_data)) / before_drop
                if drop_ratio > 0.1:
                    print(f"[factor_return] {dt} {factor_name}-{ret_col} dropna 丢弃 {before_drop - len(reg_data)}/{before_drop} ({drop_ratio:.1%})")

            n_params = base_X.shape[1] + 1  # 截距 + 因子 + 风格
            col_name = f"{factor_name}_{ret_col}"
            if len(reg_data) < n_params:
                print(f"[factor_return] {dt} 跳过 {factor_name} 对 {ret_col} 的回归：样本 {len(reg_data)} < 所需 {n_params}，置 NaN")
            ...
            try:
                ...
                # 回归质量诊断
                if model.rsquared < 0.1:
                    print(f"[factor_return] {dt} {factor_name}-{ret_col} R²={model.rsquared:.3f}，拟合质量较低")
            except Exception as e:
                print(f"[factor_return] {dt} {factor_name} 对 {ret_col} 的回归失败（{e}），置 NaN")
                coef_row[col_name] = np.nan
                t_row[col_name] = np.nan
        ...
        for ret_col in ret_list:
            ...
            for name in names:
                try:
                    ...
                    print(f"[save_performance_graphs] {factor_name}-{ret_col} 生成{name}图表失败: {e}")
            ...
            try:
                ...
                print(f"[save_performance_graphs] {factor_name}-{ret_col} 生成score_ic图表失败: {e}")
