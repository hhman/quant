...
# ---- DCF 计算工具函数 ----
def project_cashflows(base_cf: float, growth: float, years: int = PROJECTION_YEARS) -> List[float]:
...
def discount_cashflows(
    ...
    return [cf / (1 + discount_rate) ** year for year, cf in enumerate(cashflows, start=1)]
...
def terminal_value(
    ...
    return last_cf * (1 + terminal_growth) / (
...
def compute_scenario(
    ...
    terminal_pv = terminal_value(cashflows[-1], discount_rate, terminal_growth, years=years)
...
def load_financial_snapshot(path: Path) -> Optional[Dict[str, float]]:
    ...
    if pd.Timestamp.now().normalize() - report_date > pd.Timedelta(days=MAX_REPORT_AGE_DAYS):
        return None
...
def load_price_info(
    ...
    tuple[float, str, pd.Timestamp, float, pd.Timestamp, float]
]:
...
def evaluate_stock(financial_path: Path) -> Optional[ValuationSummary]:
    ...
    return ValuationSummary(
        ...
        undervaluation_pct=(
            (scenario.fair_price - latest_price) / scenario.fair_price if scenario.fair_price > 0 else 0.0
        ),
